...
public class edit_service extends AppCompatActivity {
    ...
    TextView date;
    String id, uuid, dates, odometers, names, totals, stations, notes, vehicle;
    ImageView receipt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle(Html.fromHtml("<font color='#FFFFFF' size='10'>" + "&nbsp;&nbsp;Edit Services" +"</font>"));
            ...
            public void onClick(View v) {
                try {
                    if (date.getText().toString().equals("") && odometer.getText().toString().equals("") && name.getText().toString().equals("")
                        ...
                        Toast.makeText(edit_service.this, "Please fill all the data", Toast.LENGTH_SHORT).show();
                    } else {
                        updateData();
                    }
                }catch(Exception e){
                    Toast.makeText(edit_service.this, "Enter valid data", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    void updateData(){
        service_records myDB = new service_records(edit_service.this);
        odometer_records myDB1 = new odometer_records(edit_service.this);
        ...
        myDB.updateData(id, uuid, dates, odometers, names, totals, stations, notes, vehicle, bitmap1);
    ...
    void getAndSetIntentData(){
        if(getIntent().hasExtra("id") && getIntent().hasExtra("date") &&
                getIntent().hasExtra("odometer") && getIntent().hasExtra("service") && getIntent().hasExtra("total") &&
            ...
            names = getIntent().getStringExtra("service");
            ...
            try {
                service_records myDB = new service_records(edit_service.this);
                Cursor cursor = myDB.readAllData3(id);
                if (cursor.getCount() != 0) {
                    while (cursor.moveToNext()) {
                        try {
                            activity_img.add(cursor.getBlob(9));
                        } catch (Exception e) {
