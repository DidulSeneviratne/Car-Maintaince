...
public class odometer_records extends SQLiteOpenHelper {
    ...
    public void insertData(String uuid, String date, String odometer, String notes, String vehicle){
    ...
    public long updateData(String uuid, String row_id, String date, String odometer, String notes, String vehicle){
    ...
    public long updateData1(String uuid, String date, String odometer, String notes, String vehicle){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_DATE,date);
        cv.put(COLUMN_ODOMETER,odometer);
        cv.put(COLUMN_NOTES,notes);
        cv.put(COLUMN_VEHICLES,vehicle);
        long result = db.update(TABLE_NAME, cv, "uuid=?", new String[]{uuid});
        if(result == -1){
            Toast.makeText(context, "Failed", Toast.LENGTH_SHORT).show();
            return result;
        }
    ...
    public void deleteOneRow1(String uuid){
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.execSQL("DELETE FROM " + TABLE_NAME + " WHERE " + COLUMN_SID + " = " + "'" + uuid + "'");
        }catch(Exception e){
            Toast.makeText(context, "Failed", Toast.LENGTH_SHORT).show();
        }
    }
